...
// 복합 유일 인덱스 설정
quizSchema.index(
  { grade: 1, semester: 1, subject: 1, unit: 1 },
  { unique: true }
);
...
const evaluateQuiz = async (quizData) => {
  ...
  for (const task of quizData.answers) {
    ...
    if (task.studentAnswer.trim() !== "") {
      ...
      console.log(
        `Question ID ${task.questionId} has no student answer, assigning similarity 0.`
      );
    }
  ...
  try {
    ...
    console.error(
      "Failed to notify backend server about quiz completion:",
      error
    );
  }
...
const calculateSimilarity = (studentAnswer, correctAnswer) => {
  return new Promise((resolve, reject) => {
    ...
    const result = spawnSync(
      pythonPath,
      [scriptPath, studentAnswer, correctAnswer],
      { encoding: "utf-8" }
    );
